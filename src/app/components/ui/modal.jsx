"use client"
import { useForm } from 'react-hook-form';
import React, { useState, useContext } from 'react';
import Scorecontext from '../../Context/Scorecontext';

function Modal() {
  const {
    register,
    handleSubmit,
    formState: { errors },
  } = useForm();

  const contextValue = useContext(Scorecontext);
  const { setscore } = contextValue[0];
  const { setrank } = contextValue[1];
  const { setcrct } = contextValue[2];
  const { setmodvis } = contextValue[3];
  const {graph,setgraph}= contextValue[4];

  const onSubmit = (data) => {
    
    setscore(data.percentile);
    setrank(data.rank);
    setcrct(data.score);
    setmodvis(false);
    const update_num= Math.ceil(data.percentile/10);
    const updatedData = [...graph];
    updatedData[update_num] += 1;
     setgraph(updatedData) 
  };

  return (
   
    <div className="fixed inset-0 flex items-center justify-center backdrop-blur-sm">
  <div className="p-4 w-[650px] bg-white rounded-lg shadow-md font-serif">
    <div className='w-full flex items-center justify-between p-4'><h1 className="text-lg font-semibold text-gray-700">Update scores</h1>
     <img src="data:image/png;base64," alt="" className='w-[40px] h-[30px]' />
    
    </div>
    <form onSubmit={handleSubmit(onSubmit)} className="mt-4 space-y-4">
      <div className="w-full flex items-center">
        <div className='w-[30px] h-[30px] bg-blue-800 rounded-full text-white text-lg text-center mr-2'>1</div>
        <label className="block w-1/2 text-sm font-medium text-gray-700">Update your <span className='font-semibold'>Rank</span> </label>
        <input
          type="text"
          {...register('rank', { required: 'Rank is required' })}
          className="mt-1 block w-1/2 px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
        />
      </div>
      {errors.rank && <p className="text-red-500 text-sm">{errors.rank.message}</p>}

      <div className="w-full flex items-center">
      <div className='w-[30px] h-[30px] bg-blue-800 rounded-full text-white text-lg text-center mr-2'>2</div>
        <label className="block w-1/2 text-sm font-medium text-gray-700">Update your <span className='font-semibold'>Percentile</span></label>
        <input
          type="text"
          {...register('percentile', { required: 'Percentile is required' })}
          className="mt-1 block w-1/2 px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
        />
      </div>
      {errors.percentile && <p className="text-red-500 text-sm">{errors.percentile.message}</p>}

      <div className="w-full flex items-center">
      <div className='w-[30px] h-[30px] bg-blue-800 rounded-full text-white text-xs md:text-lg text-center mr-2'>3</div>
        <label className="block w-1/2 text-sm font-medium text-gray-700">Update your <span className='font-semibold'> Current Score (out of 15)</span></label>
        <input
          type="text"
          {...register('score', { required: 'Score is required' })}
          className="mt-1 w-1/2 block px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
        />
      </div>
      {errors.score && <p className="text-red-500 text-sm">{errors.score.message}</p>}

      <div className="flex justify-end space-x-2">
        <button
          type="button"
          className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-200 rounded-md hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
          onClick={() => setmodvis(false)}
        >
          Cancel
        </button>
        <button
          type="submit"
          className="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
        >
          Save
        </button>
      </div>
    </form>
  </div>
</div>
  );
}

export default Modal;