"use client"

import React, { useContext } from 'react'
import Modal from "./modal"
import { useState } from 'react'
import Scorecontext from '../../Context/Scorecontext'
function Subject() {
  const contextValue = useContext(Scorecontext);
  const {modvid,setmodvis} = contextValue[3]
 
    
    
    return (
        
        <>
          { modvid&&<Modal></Modal>}
        <div className='w-full h-[60px] md:h-[115px] flex items-center  border border-gray-200 shadow-lg'>
            <div className='w-[20%] h-full p-2'>
              <img src="data:image/png;base64," alt="" className='w-full h-full' />
            </div>
             <div className='w-[60%] h-full flex flex-col items-center justify-center'>
                <h1 className='text-[7px] md:text-lg font-semibold font-serif'>Hypertext Markup Language</h1>
                <p className='text-[4.5px] md:text-sm font-serif'>Question:8|Duration:30Min|Sumbitted on 21 jan 2025</p>
             </div>
             <div className='w-[20%] h-full flex items-center justify-center'>
                 <button className='w-[80%] text-[8px] md:text-lg h-[25px] md:h-[40px] bg-blue-500 rounded-md'onClick={()=>{
                      setmodvis(true)
                 }}>Update</button>
             </div>

        </div>
        </>
    )
}

export default Subject
